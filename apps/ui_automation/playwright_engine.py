    ...
    async def execute_step(
        ...
        try:
            ...
            if action_type == "wait":
                ...
                screenshot_base64 = (
                    f"data:image/png;base64,{base64.b64encode(screenshot).decode()}"
                )
                execution_time = round(time.time() - start_time, 2)
                log = f"✓ 截图成功\n"
                log += f"  - 截图范围: 整个页面\n"
            ...
            if locator_strategy.lower() == "id":
                ...
                # 特别是下拉框选项，可能有多个同名选项
                if any(
                    keyword in locator_value.lower()
                    for keyword in ["dropdown", "el-select", ":has(", "li"]
                ):
                    # 如果是下拉框选项，强制只查找可见元素
                    if "visible=true" not in locator_value:
                        locator = self.page.locator(
                            f"{locator_value} >> visible=true"
                        ).first
                    else:
                ...
                if any(
                    ...
                    if "visible=true" not in locator_value:
                        locator = self.page.locator(
                            f"xpath={locator_value} >> visible=true"
                        ).first
                    else:
            ...
            if action_type == "click":
                ...
                # 通过定位器特征自动识别下拉框选项
                is_dropdown_option = (
                    "dropdown" in locator_value.lower()
                    or "el-select" in locator_value.lower()
                    or "下拉" in element_name
                    or "选项" in element_name
                    or 'role="option"' in locator_value.lower()
                    or "el-select-dropdown__item" in locator_value.lower()
                    or (  # Element Plus 下拉框
                        "//li" in locator_value and "span=" in locator_value
                    )  # XPath 下拉框模式
                )

                # 检测是否是点击 el-select 容器（下拉框触发器）
                is_select_trigger = (
                    "el-select" in locator_value.lower()
                    and "ancestor::" in locator_value.lower()
                    and "el-select-dropdown" not in locator_value.lower()
                )
                ...
                if is_select_trigger:
                    ...
                    try:
                        ...
                        wrapper_locator = locator.locator(
                            ".el-select__wrapper, input"
                        ).first
                        await wrapper_locator.click(
                            timeout=timeout_ms, no_wait_after=False
                        )
                        # 等待下拉框展开动画
                        await asyncio.sleep(0.5)
                    ...
                    try:
                        ...
                        base_locator_value = locator_value.replace(
                            " >> visible=true", ""
                        )
                        ...
                        try:
                            ...
                            if await dropdown.is_visible():
                                ...
                                await self.page.click(
                                    "body", position={"x": 10, "y": 10}, timeout=3000
                                )
                                auto_close_msg = " + 自动关闭"
                            else:
                                auto_close_msg = ""
                ...
                await locator.fill(
                    resolved_input_value, timeout=timeout_ms, force=force_action
                )
                execution_time = round(time.time() - start_time, 2)
                ...
                if resolved_input_value != step.input_value:
                    log += (
                        f"  - 变量解析: '{step.input_value}' => '{resolved_input_value}'\n"
                    )
